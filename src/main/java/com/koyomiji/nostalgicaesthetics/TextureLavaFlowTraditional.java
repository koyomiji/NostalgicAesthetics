 ...
 */
public class TextureLavaFlowTraditional extends TextureAtlasSprite {
  ...
  public TextureLavaFlowTraditional(String p_i1282_1_) {
    super(p_i1282_1_);
  }
      ...
      for (int $2 = 0; $2 < 16; $2++) {
        ...
        for (int $6 = $1 - 1; $6 <= $1 + 1; $6++) {
          for (int $7 = $2 - 1; $7 <= $2 + 1; $7++) {
            int var8 = $6 + $4 & 15;
            int var9 = $7 + $5 & 15;
            $3 += this.field_76871_g[var8 + var9 * 16];
          }
        }

        this.field_76874_h[$1 + $2 * 16] = $3 / 10.0F + (
                this.field_76875_i[($1 & 15) + ($2 & 15) * 16]
                        + this.field_76875_i[($1 + 1 & 15) + ($2 & 15) * 16]
                        + this.field_76875_i[($1 + 1 & 15) + ($2 + 1 & 15) * 16]
                        + this.field_76875_i[($1 & 15) + ($2 + 1 & 15) * 16]
        ) / 4.0F * 0.8F;
        this.field_76875_i[$1 + $2 * 16] = this.field_76875_i[$1 + $2 * 16] + this.field_76872_j[$1 + $2 * 16] * 0.01F;
        if (this.field_76875_i[$1 + $2 * 16] < 0.0F) {
          this.field_76875_i[$1 + $2 * 16] = 0.0F;
        }

        this.field_76872_j[$1 + $2 * 16] = this.field_76872_j[$1 + $2 * 16] - 0.06F;
        if (Math.random() < 0.005) {
          this.field_76872_j[$1 + $2 * 16] = 1.5F;
        }
      }
    }

    float[] $11 = this.field_76874_h;
    this.field_76874_h = this.field_76871_g;
    this.field_76871_g = $11;

    for (int $12 = 0; $12 < 256; $12++) {
      float $13 = this.field_76871_g[$12 - this.tickCounter / 3 * 16 & 0xFF] * 2.0F;
      if ($13 > 1.0F) {
        $13 = 1.0F;
      }
      ...
      imageData[$12] = TextureHelper.packRGBA($14, $15, $16, 255);
    }
